...
parser.add_argument("--use_center_loss", help="Network trained with Objectosphere + Center losses.", dest="use_center_loss", action="store_true", default=False)
...
parser.add_argument('--center_loss_weight', help='Loss weight for Center loss', type=float, default=0.1)
...
if args.Vanilla:
    ...
    callTB = TensorBoard(log_dir='logs')
    callbacks_list = [model_saver, callTB]
    ...
    info=Center_Loss_Lenet_pp.fit(
                                ...
                                batch_size=args.Batch_Size, epochs=args.no_of_epochs,verbose=1,sample_weight=[sample_weights,sample_weights,sample_weights],
                                callbacks=callbacks_list
                                )
